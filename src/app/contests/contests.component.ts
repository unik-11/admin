...
export class ContestsComponent implements OnInit {
    ...
    if (this.fixtureId) {
      ...
      this.activatedRoute.queryParamMap.subscribe(value => {
        this.filter = {
          ...
          entry_fee:value.get('entry_fee') ?? '',
          contest_category_id:value.get('contest_category_id') ?? '',
          type:value.get('type') ?? '',
        };
  ...
  async init() {
    ...
    if (value.status) {
      ...
      // if(value.data.role_id!=1 && value.data.role_id!=0){
  ...
  async getCategories() {
    ...
    if (value.status) {
      ...
      this.categories=this.categories['CRICKET'];
    }
  }
    ...
    this.fields = [
      ...
      },
      {
        ...
        fieldGroup: [
          ...
          // {
          //   hideExpression: (model, formState) => model.type !== 'BONUS',
          //   key: 'entry_fee',
          //   type: 'input',
          //   className: 'col-12 col-md-3 col-lg-3',
          //   templateOptions: {
      ...
      // {
      //   template: `<h3><strong>Contest Table</strong></h3>`,
  ...
  async generatedPdf(d) {
    ...
    if (value) {
      ...
      const v: any = await this.api.get('generatepdf/' + d.id).toPromise();
