...
@Component({
  selector: 'app-ftcontests',
  templateUrl: './ftcontests.component.html',
  styleUrls: ['./ftcontests.component.scss']
})
export class FtcontestsComponent implements OnInit {
          ...
          fieldArray: {
            ...
            fieldGroup: [
              ...
              },
              {
                ...
                expressionProperties: {
                  'model.showtotalprize': (model: any,index:any) => {
                    this.rto = parseInt(model.to) + 1;
  ...
  async totalNumberValidation(){
    // console.log(this.totalPrize)
    // debugger

    let result=(this.totalPrize==this.groundtotal)?{ 'min-number': true }:null
    return result
  }
  ...
  async generatedPdf(d) {
    ...
    if (value) {
      ...
      const v: any = await this.api.get('ftgeneratepdf/' + d.id).toPromise();
