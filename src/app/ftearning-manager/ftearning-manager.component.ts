...
@Component({
  selector: 'app-ftearning-manager',
  templateUrl: './ftearning-manager.component.html',
  styleUrls: ['./ftearning-manager.component.scss']
})
export class FtearningManagerComponent implements OnInit {

  base = 'ftbearning-manager';
  competition=[];
  search = '';
  page = 0;
  perPage=0;
  ...
  this.dataService.setTitle('Earning Manager');
    ...
    async exportData() {
      ...
      if (newValue) {
        ...
        const value = await this.api.get('ftbexport-earning-manager', { params, responseType: 'blob' }).toPromise();
        //console.log(value);
    ...
    downLoadFile(data: any, type: string) {
      ...
      if (type === 'application/vba.ms-excel') {
        a.download = 'football_earning.xlsx';
      } else {
        a.download = 'football_earning.csv';
      }
      a.click();
