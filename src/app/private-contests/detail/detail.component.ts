...
export class DetailComponent {
  ...
  constructor(private api: ApiService,
    ...
    this.activatedRoute.paramMap.subscribe(value => {
      if(value.get('privateContestId') && value.get('userId')) {
        this.privateContestId = value.get('privateContestId');
        this.userId = value.get('userId');
        this.search = value.has('search') ? value.get('search') : '';
        this.init().then();
        this.getUserTeams().then();
      }else if (value.get('privateContestId')) {
        this.privateContestId = value.get('privateContestId');
        this.search = value.has('search') ? value.get('search') : '';
        this.init().then();
        this.getUserTeams().then();
  ...
  async init() {
    ...
    const value: any = await this.api.get(this.base + '/' + this.privateContestId, {params}).toPromise();
    if (value.status) {
  ...
  async getUserTeams() {
    ...
    params = params.set('contest_id', this.privateContestId);
