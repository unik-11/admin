...
@Component({
  selector: 'app-bkbleaderboard-details',
  templateUrl: './bkbleaderboard-details.component.html',
  styleUrls: ['./bkbleaderboard-details.component.scss']
})
export class BkbleaderboardDetailsComponent implements OnInit {
  base='bkbleaderboard-detail';
  checked = true;
  page = 0;
  search = '';
  perPage=0;
  ...
  async setParams() {
    ...
    await this.router.navigate(['/bkbleaderboard/'+this.leaderboardId+"/"], { queryParams: params });
    await this.init();
