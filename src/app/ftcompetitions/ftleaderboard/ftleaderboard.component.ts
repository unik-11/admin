...
@Component({
  ...
  templateUrl: './ftleaderboard.component.html',
  styleUrls: ['./ftleaderboard.component.scss']
})
export class FtleaderboardComponent implements OnInit {
  base='ftleaderboard';
  checked = true;
  page = 0;
  search = '';
  ...
  async setParams() {
    ...
    await this.router.navigate(['/ftleaderboard/'+this.competitionId+"/"], { queryParams: params });
    await this.init();
  ...
  async exportData() {
    ...
    if (newValue) {
      ...
      const value = await this.api.get('ftbexport-leaderboard-data', { params, responseType: 'blob' }).toPromise();
      //console.log(value);
