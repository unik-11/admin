...
export class ViewcronComponent implements OnInit {
  ...
  async init() {
    ...
    params = params.set('type', "cricket");
  ...
  async cronDeleteAll() {
    const value: any = await this.formService.confirm("Confirmation", "Are you sure to delete all cron?");
    if (value) {
      let params = new HttpParams();
      params = params.set('flag', 'delete_all_cron');
      params = params.set('type', 'cricket');
