...
export class FixturesComponent {
  ...
  @ViewChild('exportTemplate') exportTemplate: TemplateRef<HTMLElement>; live
  perPage = 0;
  liveperPage = 0;
  upcperPage = 0;
  compperPage = 0;
  ...
  types = ['NOT STARTED', 'LIVE', 'IN REVIEW', 'COMPLETED', 'CANCELED'];
  ...
  async getCompData() {
    ...
    if (value.status) {
      ...
      this.addFixtureCommission = value.data.addFixtureCommission;
  ...
  async cronCheck(d) {
    ...
    const value: any = await this.formService.show(fields, 'viewcron/'+d.id, 'POST', Object.assign({}, d), {
      title: 'Cron Run',
      width: '350px'
    });
  ...
  checkCommissionAdd(d){
    console.log(this.addFixtureCommission.filter(f=>f.fixture_id == d).length > 0)
    return this.addFixtureCommission.filter(f=>f.fixture_id == d).length > 0;
  }
